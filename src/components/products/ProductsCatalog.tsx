  ...
  return (
    <div className="container mx-auto px-4 py-12 md:py-16">
      <div className="text-center mb-12">
        <h1 className="text-4xl font-bold tracking-tight font-headline">{t("ourProducts")}</h1>
        <p className="mt-4 max-w-2xl mx-auto text-lg text-muted-foreground">{t("productsPage.subtitle")}</p>
      </div>
          ...
          <div className="sticky top-24">
            ...
            <div>
              ...
              <Slider
                ...
                max={1500}
                step={50}
                onValueChange={(value) => setPriceRange([value[0] ?? 0, value[1] ?? 1500])}
              />
        ...
        <div className="md:col-span-3">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center bg-card p-8 rounded-lg">
              <p className="text-xl font-semibold">{t("productsPage.noProductsFound")}</p>
              <p className="text-muted-foreground mt-2">{t("productsPage.adjustFilters")}</p>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <Card className="flex flex-col overflow-hidden transition-all duration-300 hover:border-primary hover:shadow-lg hover:shadow-primary/20">
      <CardHeader className="p-0 relative">
        ...
        {requiresUpload && (
          <Badge variant="destructive" className="absolute top-2 right-2 text-[11px]">
      ...
      <CardContent className="flex-1 p-4">
        ...
        {productDescription && (
          <p className="text-sm text-muted-foreground mt-2 line-clamp-3">{productDescription}</p>
        )}
      </CardContent>
      <CardFooter className="flex items-center justify-between p-4 bg-background/30">
        <p className="text-lg font-bold text-primary">{formatCurrency(service.precoBase)}</p>
        <Button
          size="sm"
          onClick={() => {
